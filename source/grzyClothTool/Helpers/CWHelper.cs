...
public static class CWHelper
{
    ...
    public static bool IsPreviewAvailable => GTAFolder.IsCurrentGTAFolderValid();
    ...
    public static void Init()
    {
        ...
        {
            LogHelper.Log(
                LocalizationHelper.Translate("Путь GTA V не настроен. 3D-просмотр будет доступен после выбора пути в настройках."),
                Views.LogType.Warning);
        }
    }
    ...
    {
        string extension = Path.GetExtension(texture.FullFilePath).ToLowerInvariant();
        byte[] data = extension switch
        {
            ...
            _ => throw new NotSupportedException($"Неподдерживаемое расширение файла: {texture.Extension}"),
        };
