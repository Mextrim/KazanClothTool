...
public static class DragDropHelper
{
    ...
    public static async Task<List<string>> ExtractVirtualFilesAsync(IDataObject data, Func<string, bool>? fileExtensionFilter = null)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < fileCount; i++)
            {
                try
                {
                    ...
                    {
                        LogHelper.Log($"Не удалось извлечь файл {i + 1} ({safeFilename}): {comEx.Message}", LogType.Error);
                    }
                ...
                {
                    LogHelper.Log($"Ошибка извлечения файла {i + 1}: {ex.Message}", LogType.Error);
                    continue;
                }
            }
        }
        ...
        {
            LogHelper.Log($"Ошибка в ExtractVirtualFilesAsync: {ex.Message}", LogType.Error);
        }
    ...
    public static bool CheckForFilesInDescriptor(IDataObject data, Func<string, bool> fileExtensionFilter)
    {
        ...
        {
            LogHelper.Log($"Ошибка проверки FileGroupDescriptor: {ex.Message}", LogType.Error);
            return false;
        }
    }
