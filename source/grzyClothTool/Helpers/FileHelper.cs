...
public static class FileHelper
{
    ...
    /// </summary>
    private static string? _loadContextProjectFolder;
    private static string? _currentProjectRoot;

    public static string? CurrentProjectRoot => _currentProjectRoot;

    public static void SetCurrentProjectRoot(string? projectFolder)
    {
        _currentProjectRoot = string.IsNullOrWhiteSpace(projectFolder)
            ? null
            : Path.GetFullPath(projectFolder);
        _loadContextProjectFolder = _currentProjectRoot;
    }
    ...
    public static void SetLoadContext(string saveFilePath)
    {
        try
        {
            ...
            {
                SetCurrentProjectRoot(saveFileDir);
            }
        }
        catch
        {
    ...
    {
        string projectRoot = _currentProjectRoot ?? GetProjectRoot(
            PersistentSettingsHelper.Instance.MainProjectsFolder,
            MainWindow.AddonManager.ProjectName);

        string assetsPath = Path.Combine(projectRoot, GlobalConstants.ASSETS_FOLDER_NAME);
        Directory.CreateDirectory(assetsPath);
        return assetsPath;
    }

    public static string GetProjectRoot(string projectsFolder, string projectName)
    {
    ...
    public static string ResolveFilePath(string path)
    {
        ...
        {
            LogHelper.Log($"Ошибка определения пути файла «{path}»: {ex.Message}", Views.LogType.Warning);
            return path;
        }
    }
    ...
    public static async Task<string> CopyToProjectAssetsAsync(string sourceFilePath, string guid)
    {
        ...
        await CopyFileAtomicallyAsync(sourceFilePath, destinationPath);
        return fileName;
    }
    ...
    private static async Task CopyFileAtomicallyAsync(string sourceFilePath, string destinationPath)
    {
        ...
        try
        {
            await CopyAsync(sourceFilePath, temporaryPath);
            var copiedFile = new FileInfo(temporaryPath);
            if (!copiedFile.Exists || copiedFile.Length == 0)
            {
                throw new IOException($"Скопированный файл пуст: {sourceFilePath}");
            }
    ...
    public static async Task<GDrawable> CreateDrawableAsync(string filePath, Enums.SexType sex, bool isProp, int typeNumber, int countOfType)
    {
        ...
        else
        {
            ...
            {
                LogHelper.Log($"Не удалось скопировать одежду в ресурсы проекта: {ex.Message}. Используется исходный путь.", LogType.Warning);
    ...
    public static void OpenFileLocation(string path)
    {
        ...
        {
            ErrorLogHelper.LogError("Не удалось открыть расположение файла", ex);
            CustomMessageBox.Show(
                LocalizationHelper.Format("Не удалось открыть расположение файла: {0}", ErrorMessageHelper.Friendly(ex)),
    ...
    public static async Task SaveTexturesAsync(List<GTexture> textures, string folderPath, string format)
    {
        ...
        string fileExtension = format.ToUpper() switch
        {
            ...
            "PNG" => ".png",
            "YTD" => ".ytd",
            _ => throw new ArgumentException($"Неподдерживаемый формат: {format}", nameof(format))
        };
