...
namespace grzyClothTool.Views
{
    ...
    public partial class OptimizeWindow : Window, INotifyPropertyChanged
    {
            ...
            set
            {
                switch (value)
                {
                    case "DXT1 (без альфа)":
                        ...
                        break;
                    case "DXT3 (резкая альфа)":
                        _textureFormat = TextureFormat.D3DFMT_DXT3;
                        break;
                    case "DXT5 (градиентная альфа)":
        ...
        public string[] AvailableCompression { get; set; } = ["DXT1 (без альфа)", "DXT3 (резкая альфа)", "DXT5 (градиентная альфа)"];
        ...
        private void OptimizeTexture_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Close();
                CustomMessageBox.Show(LocalizationHelper.Translate("Параметры оптимизации не выбраны"), "Предупреждение", Controls.CustomMessageBox.CustomMessageBoxButtons.OKOnly, Controls.CustomMessageBox.CustomMessageBoxIcon.Warning);
                return;
            }
            ...
            LogHelper.Log(LocalizationHelper.Translate("Текстуры будут оптимизированы при сборке ресурса"));
            Close();
        }
