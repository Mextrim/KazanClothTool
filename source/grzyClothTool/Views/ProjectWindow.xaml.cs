...
namespace grzyClothTool.Views
{
    ...
    public partial class ProjectWindow : UserControl, INotifyPropertyChanged
    {
        ...
        private async void Add_DrawableFile(object sender, RoutedEventArgs e)
        {
            ...
            {
                CustomMessageBox.Show($"Не удалось добавить одежду: {ex.Message}", "Ошибка", CustomMessageBox.CustomMessageBoxButtons.OKOnly, CustomMessageBox.CustomMessageBoxIcon.Error);
            }
            finally
            {
        ...
        private async void Add_DrawableFolder(object sender, RoutedEventArgs e)
        {
            ...
            bool success = false;
            int allFilesCount = 0;
            ProgressHelper.Start("Поиск одежды...");
            try
            {
            ...
            {
                ProgressHelper.Stop(success ? $"Добавлено файлов: {allFilesCount:0}" : "Не удалось добавить одежду", success);
            }
        }
        ...
        private void Delete_SelectedDrawable(object sender, RoutedEventArgs e)
        {
            ...
            string message = count == 1
                ? $"Удалить выбранный элемент одежды «{Addon.SelectedDrawable?.Name}»?"
                : $"Удалить выбранные элементы одежды ({count})?";
            message += "\n\nНумерация последующих элементов изменится.\n\nЗаменить их зарезервированными слотами вместо удаления?";
        ...
        private async void DrawablesGroupBox_Drop(object sender, System.Windows.DragEventArgs e)
        {
            ...
            {
                LogHelper.Log($"Ошибка при добавлении перетащенных файлов: {ex.Message}", LogType.Error);
