...
namespace grzyClothTool.Controls
{
    public class DuplicateBatchItem : INotifyPropertyChanged
    {
        ...
        public string NewDrawableName => System.IO.Path.GetFileName(Drawable?.FilePath) ?? "Неизвестно";
        ...
        public string DuplicateOfText
        {
            get
            {
                if (ExistingDuplicates == null || ExistingDuplicates.Count == 0)
                    return "Совпадающих элементов не найдено";

                var names = ExistingDuplicates.Select(d => d.Name).Take(3);
                var text = "Дубликат: " + string.Join(", ", names);
                if (ExistingDuplicates.Count > 3)
                    text += $" (ещё {ExistingDuplicates.Count - 3})";
                return text;
            }
        }

        public string TypeDisplay => Drawable?.IsProp == true ? "Пропс" : "Компонент";
    ...
    public partial class DuplicateBatchDialog : Window
    {
        ...
        private void UpdateSummary()
        {
            ...
            SummaryText.Text = $"Будет добавлено дубликатов: {selectedCount} из {_items.Count}";
        }
