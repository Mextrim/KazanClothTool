...
public class GDrawable : INotifyPropertyChanged
{
    ...
    public string SexName
    {
        get
        {
            return _sexName ??= Enum.GetName(Sex) ?? string.Empty;
        }
        set
        {
            if (_sexName != value)
    ...
    private GDrawableDetails _details = new();
    ...
    private string? _firstPersonPath;
    public string? FirstPersonPath
    {
        get => _firstPersonPath;
        set
        {
            if (_firstPersonPath != value)
            {
                _firstPersonPath = value;
                OnPropertyChanged();
                OnPropertyChanged(nameof(FullFirstPersonPath));
            }
        }
    }
    ...
    private string? _clothPhysicsPath;
    public string? ClothPhysicsPath
    {
        get => _clothPhysicsPath;
        set
        {
            if (_clothPhysicsPath != value)
            {
                _clothPhysicsPath = value;
                OnPropertyChanged();
                OnPropertyChanged(nameof(FullClothPhysicsPath));
            }
        }
    }
    ...
    public ObservableCollection<SelectableItem> SelectedFlags
    {
        ...
        set
        {
            if (ReferenceEquals(_selectedFlags, value))
            {
                return;
            }

            if (_selectedFlags != null)
            {
                _selectedFlags.CollectionChanged -= OnSelectedFlagsCollectionChanged;
                foreach (var item in _selectedFlags)
                {
                    item.PropertyChanged -= OnSelectedFlagItemPropertyChanged;
                }
            }
    ...
    public string FlagsText
    {
        get
        {
            ...
            return count > 0 ? $"{Flags} (выбрано: {count})" : "НЕТ";
        } 
    }
    ...
    public GDrawable(Guid id, string filePath, Enums.SexType sex, bool isProp, int typeNumeric, int number, bool hasSkin, ObservableCollection<GTexture> textures)
    {
        ...
        SelectedFlags.CollectionChanged += OnSelectedFlagsCollectionChanged;
        ...
        {
            LogHelper.Log($"Не удалось найти файл одежды «{Name}»: {ex.Message}", Views.LogType.Warning);
            IsLoading = false;
        }
    }

    public void RestorePersistedEmbeddedTextures(string projectRoot, GDrawableDetails target = null)
    {
        if (target == null)
        {
            target = Details;
        ...
        if (target?.EmbeddedTextures == null)
        {
            return;
        }

        foreach (var embeddedTexture in target.EmbeddedTextures.Values)
        {
            embeddedTexture?.TryRestorePersistedTexture(projectRoot);
        }
    }
    ...
    public async Task LoadDetails()
    {
        ...
        try
        {
            if (File.Exists(fullPath))
            {
                ...
                if (result != null)
                {
                    ...
                    if (previousDetails?.EmbeddedTextures != null)
                    {
                        foreach (var pair in previousDetails.EmbeddedTextures)
                        {
                            var previousTexture = pair.Value;
        ...
        {
            Helpers.ErrorLogHelper.LogError($"Ошибка загрузки сведений об элементе одежды {Name}: {ex.Message}", ex);
        }
        finally
        {
            IsLoading = false;
    ...
    private static async void ProcessLoadQueue()
    {
        foreach (var drawable in _loadQueue.GetConsumingEnumerable())
        {
            ...
            catch (Exception ex)
            {
                ...
                Helpers.ErrorLogHelper.LogError($"Не удалось загрузить сведения об элементе одежды {drawable.Name}: {ex.Message}", ex);
    ...
    private void OnSelectedFlagsCollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        if (e.OldItems != null)
        {
    ...
    private async Task<GDrawableDetails?> GetDrawableDetailsAsync(string filePath)
    {
        ...
        {
            ErrorLogHelper.LogError($"Не удалось загрузить файл одежды «{Name}»: {ex.Message}", ex);
            return null;
        }
