...
namespace CodeWalker
{
    public static class GTAFolder
    {
        ...
        public static bool ValidateGTAFolder(string folder, out string failReason)
        {
            ...
            {
                failReason = "Папка не указана";
                return false;
            }

            if(!Directory.Exists(folder))
            {
                failReason = $"Папка \"{folder}\" не существует";
                return false;
            }

            if(!File.Exists(folder + @"\gta5.exe"))
            {
                failReason = $"Файл GTA5.exe не найден в папке \"{folder}\"";
                return false;
            }
        ...
        public static bool UpdateGTAFolder(bool UseCurrentIfValid = false)
        {
            ...
            if(ValidateGTAFolder(folder, out failReason))
            {
                ...
                {
                    MessageBox.Show($"Папка GTA успешно изменена на \"{folder}\"", "Папка GTA", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                return true;
            } else
            {
                var tryAgain = MessageBox.Show($"Папка \"{folder}\" не подходит для GTA:\n\n{failReason}\n\nПопробовать выбрать другую папку?", "Не удалось выбрать папку GTA", MessageBoxButtons.RetryCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
